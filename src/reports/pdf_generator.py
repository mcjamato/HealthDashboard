...
class PDFReportGenerator:
    def create(self, name, sections, correlations) -> bytes:
        ...
        story = [
            Paragraph("Health & Wellness Report", styles["Title"]),
        ...
        for title, frame in sections:
            ...
            if frame.empty:
                story.append(
                    Paragraph("No records available.", styles["BodyText"])
                )
            else:
                preview = frame.head(10).drop(
        ...
        story.append(
            Paragraph(
                (
                    "Available in the application analytics page."
                    if not correlations.empty
                    else "Not enough data for correlations."
                ),
                styles["BodyText"],
            )
        )
